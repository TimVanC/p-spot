...
export default function ScoreRevealScreen() {
  ...
  const buildShareMessage = () => {
    const h = scoreResult.bonus_hydration;
    const hydrationLabel = h > 3 ? 'Elite 💧' : h > 1 ? 'Solid 💧' : 'Drink water 🚰';
    const altLine = exifData?.altitudeFt ? `${exifData.altitudeFt}ft` : 'Unknown';
    const wildlifeLine = scoreResult.wildlife_detected ? '🦅 Wildlife detected\n' : '';
    const peeLine = scoreResult.pee_detected ? '💧 Stream confirmed\n' : '❌ No stream detected\n';
    ...
    return [
      ...
      `Elevation: ${altLine} 🏔️`,
      `Hydration: ${hydrationLabel}`,
      `Danger: ${scoreResult.bonus_danger}/8 ⚡`,
      `${wildlifeLine}${peeLine}――――――――――――――――`,
